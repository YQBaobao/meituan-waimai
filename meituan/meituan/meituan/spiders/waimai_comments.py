...
from meituan.cookie import structure_cookie
...
class WaimaiCommentsSpider(RedisSpider):
    name = "waimai_comments"
    redis_key = "meituan:comments"
    ...
    def make_request_from_data(self, data):
        ...
        cookies = structure_cookie()
        url = "https://i.waimai.meituan.com/openh5/poi/comments?_={}".format(self.ts)
        form_data = [{
            "lng": str(int(long) / 1000000),
            "lat": str(int(lat) / 1000000),
            "gpsLng": str(int(long) / 1000000),
            "gpsLat": str(int(lat) / 1000000),
            "shopId": "0",
            "mtWmPoiId": shop_id,
        ...
        for data in form_data:
            return FormRequest(url, formdata=data, headers=headers, dont_filter=True, cookies=cookies)
